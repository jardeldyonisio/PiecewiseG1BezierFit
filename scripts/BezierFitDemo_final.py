...
def adaptive_knots_placement(Q, initial_knots=3, max_knots=20, error_threshold=0.05, max_iterations=10):
    """
    Adaptive knot placement algorithm that iteratively adds knots where error is highest.
    This is the most advanced approach for optimal knot selection.
    
    Parameters:
    Q: trajectory points (Nx2)
    initial_knots: starting number of knots
    max_knots: maximum number of knots allowed
    error_threshold: target error threshold to stop iteration
    max_iterations: maximum number of iterations to prevent infinite loops
    
    Returns:
    best_k: optimal knot indices (1-based for MATLAB compatibility)
    error_history: history of errors for analysis
    iteration_info: detailed information about each iteration
    """
    
    print(f"=== ADAPTIVE KNOT PLACEMENT ===")
    print(f"Target error threshold: {error_threshold}")
    print(f"Max knots allowed: {max_knots}")
    ...
    for iteration in range(max_iterations):
        ...
        print(f"Current knots: {current_knots} (n={len(current_knots)})")
        ...
        try:
            ...
            segment_errors = calculate_segment_errors(GOC, Qt, current_knots, dpkpc)
...
def calculate_segment_errors(GOC, Q, k, dpkpc):
    """
    Calculate the error for each Bezier segment.
    """
    try:
        # Get control points
        from cpoints import cpoints
        ...
        for i in range(len(k) - 1):
            ...
            if ctrl_start + 3 < C.shape[1]:
                ...
                if segment_data.shape[1] > 0:
                    error = calculate_segment_distance_error(ctrl_points, segment_data)
...
def calculate_segment_distance_error(ctrl_points, data_points):
    """
    Calculate the sum of minimum distances from data points to Bezier curve segment.
    """
    try:
...
def BezierFitDemo_final():
    ...
    if demo == 1:
        ...
        print("=== STARTING ADAPTIVE KNOT PLACEMENT ===")
        k_adaptive, error_history, iteration_info = adaptive_knots_placement(
            Q, 
            initial_knots=3, 
            max_knots=15, 
...
Status: {status}
    ...
    plt.tight_layout()
    plt.savefig('/home/jardeldyonisio/PiecewiseG1BezierFit/python_adaptive_analysis.png', dpi=300, bbox_inches='tight')
    print("Saved Python adaptive analysis plot")
